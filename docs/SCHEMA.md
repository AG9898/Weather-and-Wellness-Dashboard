...
> workflow on a push to `main` that touches `backend/**` — see
> [`docs/DEPLOYMENT.md`](DEPLOYMENT.md). Applying by hand is only needed when
> that workflow is bypassed or its migration step fails.
...
| 2026-09-03 | T1849                 | Migration `20260903_000001`: add `misokinesia_participants.language`; rewrite miso demographics choice columns from English display text to option keys                                                              |


As of 2026-09-03, migration `20260903_000001` is the current head revision,
applied to production by the Production Release workflow on 2026-09-03.

### Applying Migrations

A push to `main` that touches `backend/**` runs the Production Release workflow,
which deploys Railway and then runs `scripts/alembic-upgrade-head.sh` against
production. In the normal path there is nothing to apply by hand.

Note the ordering in that workflow: the backend is deployed **before** the
migration runs, so between those two steps the new code is live against the old
schema. For a migration that adds a `NOT NULL` column the ORM already selects,
or otherwise rewrites data in place, that window will return errors on the
affected endpoints until the migration lands. Keep such migrations small, and
prefer a release window with low participant traffic.

If the workflow is bypassed or its migration step fails, apply from a host with
database access:
...
Do not run Alembic through the transaction pooler on port `6543`. Note that some
development machines have no network egress on port `5432` and cannot reach the
pooler or direct host at all; on those, rely on the release workflow.
