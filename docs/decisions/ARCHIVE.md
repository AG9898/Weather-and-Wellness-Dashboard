...
> Historical resolved decision bodies moved out of `docs/DECISIONS.md` during
> documentation cleanup. Use `docs/DECISIONS.md` for open decisions and current
> routing. This archive preserves the original resolved decision text for audit
> and background context.
>
> Note: some historical IDs were duplicated before this cleanup. Do not assign
> new IDs by scanning this archive alone; use the template and current register
...
### RESOLVED-21 — RA Chatbot May Fall Back to a Relaxed-Privacy Model for Availability

**Resolved:** 2026-06-19

**Decision:** The RA chatbot keeps ZDR and provider training/data-collection
opt-out enforced on its primary OpenRouter model but may, **optionally**, fall
back to a relaxed-privacy model for availability when the primary ZDR-required
request fails due to provider unavailability/upstream error. The fallback is
opt-in via `OPENROUTER_FALLBACK_MODEL`; when unset, the client continues to fail
closed. Recommended fallback slug: `nvidia/nemotron-3-super-120b-a12b:free`.
...
they all route through Venice. Free Gemma/Nemotron endpoints are not ZDR, and
OpenRouter may also reject them when free-model training/data-collection is
denied. To keep the RA-facing feature usable when Venice is down or rate-limited,
the owner accepts relaxing ZDR and provider training/data-collection filtering
for the fallback request only.
...
- The primary path always keeps ZDR and `data_collection="deny"` enforced; only
  the single fallback retry relaxes provider routing by omitting ZDR and sending
  `data_collection="allow"`. `OPENROUTER_FALLBACK_PROVIDER_ALLOWLIST` may
  optionally scope the fallback.
- The served route (primary ZDR vs. relaxed-privacy fallback) must be observable
  for research-ethics auditing, without exposing secrets.
...
`docs/SCHEMA.md`, `docs/labs/weather-wellness/weather/DESIGN_SPEC.md`, and `docs/devSteps.md` before implementation.
...
**Affects:** docs/labs/weather-wellness/weather/WEATHER_INGESTION.md, docs/SCHEMA.md, docs/labs/weather-wellness/weather/API.md, docs/labs/weather-wellness/weather/DESIGN_SPEC.md, backend weather/session day-linking logic, Phase 3 import/start-session implementations.
...
**Affects:** docs/labs/weather-wellness/weather/DESIGN_SPEC.md, docs/labs/weather-wellness/weather/API.md (`POST /sessions/start`), docs/SCHEMA.md (`participants` columns), frontend dashboard start flow.
...
**Affects:** T05 (SMALLINT range 1-5), T10 (scoring module), T16 (frontend screen). See docs/labs/weather-wellness/weather/COGFUNC8A.md for full specification. Updated docs/labs/weather-wellness/weather/SCHEMA.md and docs/labs/weather-wellness/weather/SCORING.md.
...
**Affects:** T10 (scoring module), T15 (frontend CES-D screen). See docs/labs/weather-wellness/weather/CESD10.md for full specification. Updated docs/labs/weather-wellness/weather/SCHEMA.md and docs/labs/weather-wellness/weather/SCORING.md.
...
**Affects:** docs/PRD.md, docs/SCHEMA.md, docs/labs/weather-wellness/weather/API.md, docs/labs/weather-wellness/weather/DESIGN_SPEC.md.
...
**Affects:** docs/labs/weather-wellness/weather/DESIGN_SPEC.md, docs/labs/weather-wellness/weather/API.md.
...
**Affects:** docs/labs/weather-wellness/weather/API.md, docs/SCHEMA.md, docs/labs/weather-wellness/weather/DESIGN_SPEC.md,
