...
| `/import-export` | RA required (via `(ra)` layout), currently IHTT action-gated | `canonical` | Shared Import/Export route. For the current IHTT rollout it is visible only to IHTT users and exposes Poffenberger XLSX export only; Weather-Wellness legacy data ops are not exposed through this route for non-IHTT users. |
...
- **Frontend auth gate (two layers):**
  1. `src/middleware.ts` — Next.js edge middleware using `@supabase/ssr` `createServerClient`. Runs before any page rendering; redirects unauthenticated requests to `/login?next=<path>` for all RA routes (`/dashboard`, `/new-session`, `/import-export`, `/misokinesia`, `/ihtt`, `/users`). This is the primary server-side gate.
  2. `src/app/(ra)/layout.tsx` — client-side secondary guard. Handles mid-session sign-outs and populates `RAUserContext` (`email` from the Supabase Auth user plus `role` and `lab_name` from `app_metadata`) after hydration. Individual lab/action pages may add client-side lab gates for navigation hygiene, but backend route dependencies remain authoritative.
...
- **Post-login redirect:** After successful login, `LoginDialogForm` reads the `?next=` search param and redirects there, falling back to the authenticated user's lab landing page from Supabase `app_metadata.lab_name` (`/dashboard` for `ww`, `/ihtt/poffenberger` for `ihtt`).
