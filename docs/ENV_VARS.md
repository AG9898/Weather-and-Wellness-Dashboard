...
| `OPENROUTER_FALLBACK_MODEL` | Optional (RA chatbot) | — | Relaxed-privacy model slug used **only** when the primary ZDR-required request fails due to provider unavailability/upstream error (not misconfiguration). Setting it is a deliberate, owner-approved relaxation of ZDR and provider training/data-collection opt-out for the fallback request to improve availability when the sole free ZDR provider (Venice) is down or rate-limited. Unset = no fallback (fail closed). Recommended: `nvidia/nemotron-3-super-120b-a12b:free`. See `docs/AI_CHAT.md` and `docs/DECISIONS.md`. | OpenRouter model catalog |
