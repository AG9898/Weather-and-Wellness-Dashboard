    ...
    <div className="relative min-h-screen overflow-hidden bg-[linear-gradient(180deg,#001328_0%,#001a41_34%,#002455_68%,#001328_100%)] text-white">
      ...
      <div className="relative z-10 flex min-h-screen items-center justify-center px-4 py-8">
        <div ref={heroRef} className="mx-auto flex w-full max-w-6xl flex-col items-center text-center">
          ...
          <h1 className="max-w-6xl text-[clamp(2.95rem,9vw,7.15rem)] font-bold leading-[0.9] tracking-[-0.055em]">
            {titleLetters.map((line, lineIndex) => (
              <span
                key={`line-${lineIndex}`}
                className="block whitespace-nowrap last:mt-2 sm:last:mt-3"
              >
                {line.map((word, wordIndex) => (
                  <span
                    key={TITLE_LINES[lineIndex][wordIndex]}
                    className="mr-3 inline-block last:mr-0 sm:mr-5 md:mr-6"
                  >
                    {word.map(({ id, letter }) => (
                      <span
                        key={id}
          ...
          <div
            data-hero-cta
            className="mt-8 inline-block rounded-[1.4rem] border border-white/12 bg-gradient-to-b from-white/14 to-white/4 p-[1px] shadow-[0_24px_80px_rgba(0,0,0,0.35)] backdrop-blur-xl sm:mt-10"
          >
            <Button
              type="button"
